...
CREATE TABLE assessment_catalog (
    ...
    updated_at timestamptz NOT NULL DEFAULT now(),
    CHECK (
        NOT enabled_globally
        OR digital_right_status IN ('allowed', 'allowed_after_governance_approval')
    )
);
...
CREATE TABLE institution_assessment_authorizations (
    ...
    status text NOT NULL CHECK (
        status IN ('pending', 'approved', 'restricted', 'suspended', 'expired', 'rejected')
    ),
    authorized_operations jsonb NOT NULL DEFAULT '[]'::jsonb CHECK (
        jsonb_typeof(authorized_operations) = 'array'
    ),
...
CREATE TABLE provider_assessment_authorizations (
    provider_authorization_id text PRIMARY KEY CHECK (
        provider_authorization_id ~ '^PAUTH-[A-Z0-9-]{8,50}$'
    ),
    institution_id text NOT NULL REFERENCES institutions(institution_id),
    provider_id text NOT NULL,
    assessment_id text NOT NULL REFERENCES assessment_catalog(assessment_id),
...
CREATE TABLE cases (
    ...
    education_languages jsonb NOT NULL DEFAULT '[]'::jsonb CHECK (
        jsonb_typeof(education_languages) = 'array'
    ),
...
CREATE TABLE consent_versions (
    ...
    created_by_provider_id text NOT NULL,
    created_at timestamptz NOT NULL DEFAULT now(),
    supersedes_consent_version_id text,
...
CREATE TABLE safety_event_reviews (
    ...
    decision text NOT NULL CHECK (
        decision IN ('continue_hold', 'resume_pathway', 'close_case', 'escalate')
    ),
...
CREATE TABLE pathway_instances (
    ...
    status text NOT NULL CHECK (
        status IN ('active', 'safety_hold', 'completed', 'withdrawn', 'superseded')
    ),
...
CREATE TABLE pathway_events (
    ...
    institution_id text NOT NULL REFERENCES institutions(institution_id),
    case_id text NOT NULL,
    pathway_instance_id text NOT NULL,
...
CREATE TABLE assessment_sessions (
    ...
    status text NOT NULL CHECK (
        status IN ('planned', 'in_progress', 'completed', 'stopped', 'invalidated')
    ),
    validity text NOT NULL CHECK (
        validity IN ('not_yet_determined', 'valid', 'qualified', 'invalid')
    ),
...
CREATE TABLE team_reviews (
    ...
    pathway_instance_id text NOT NULL,
    status text NOT NULL CHECK (
        status IN ('draft', 'approved', 'insufficient_evidence', 'returned_for_data')
    ),
    ...
    supporting_evidence_ids jsonb NOT NULL DEFAULT '[]'::jsonb CHECK (
        jsonb_typeof(supporting_evidence_ids) = 'array'
    ),
    contrary_evidence_ids jsonb NOT NULL DEFAULT '[]'::jsonb CHECK (
        jsonb_typeof(contrary_evidence_ids) = 'array'
    ),
...
CREATE TABLE report_versions (
    ...
    created_by_provider_id text NOT NULL,
    created_at timestamptz NOT NULL DEFAULT now(),
    supersedes_report_version_id text,
...
BEGIN
    ...
    LOOP
        policy_name := 'institution_isolation_' || target_table;
        EXECUTE format('ALTER TABLE %I ENABLE ROW LEVEL SECURITY', target_table);
        EXECUTE format('ALTER TABLE %I FORCE ROW LEVEL SECURITY', target_table);
        EXECUTE format(
            'CREATE POLICY %I ON %I
             USING (institution_id = nullif(current_setting(''app.institution_id'', true), ''''))
             WITH CHECK (institution_id = nullif(current_setting(''app.institution_id'', true), ''''))',
            policy_name,
            target_table
        );
    END LOOP;
...
BEGIN
    ...
    LOOP
        policy_name := 'assigned_case_access_' || target_table;
        EXECUTE format(
            'CREATE POLICY %I ON %I AS RESTRICTIVE FOR ALL
             USING (
                 EXISTS (
                     ...
                     WHERE accessible_case.institution_id = %I.institution_id
                       AND accessible_case.case_id = %I.case_id
                 )
             )
             WITH CHECK (
                 EXISTS (
...
'Provider assessment workflow only. Identity is referenced through an external vault. Protected test content and norm tables are prohibited.';
...
'Opaque reference to an approved scoring or publisher boundary. Protected items, prompts, answer keys, and norm tables are prohibited.';

COMMENT ON COLUMN report_versions.content_reference IS
'Opaque reference to immutable, versioned report content. Signed content is identified by content_hash.';
