...
SCRIPT = ROOT / "scripts" / "publish_accessible_mental_health_appointments_v194.py"

spec = importlib.util.spec_from_file_location("accessible_appointments_v194", SCRIPT)
module = importlib.util.module_from_spec(spec)
assert spec.loader is not None
spec.loader.exec_module(module)


class AccessibleMentalHealthAppointmentsTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.data = json.loads(CONTENT.read_text(encoding="utf-8"))

    def test_content_depth_and_safety(self):
        ...
        for marker in ["الحاجز", "التكييف", "المرافق", "الحمل الحسي", "الموافقة", "الطوارئ", "built-not-published"]:
            if marker != "built-not-published":
                self.assertIn(marker, text)
        for forbidden in ["يشفي", "يضمن العلاج", "بديل عن التقييم"]:
            self.assertNotIn(forbidden, self.data["title"] + self.data["summary"])
    ...
    def test_sources_follow_contract(self):
        ...
        for source in sources:
            ...
            self.assertIn(source["source_type"], {"official_report", "official_guideline"})
    ...
    def test_render_metadata_schema_and_links(self):
        with tempfile.TemporaryDirectory() as temporary:
            site = Path(temporary)
            for relative in ["special-needs/index.html", "guides/index.html", "start-here/index.html"]:
                path = site / relative
            ...
            href = "/special-needs/accessible-mental-health-appointments-for-disabled-people/"
            for relative in ["special-needs/index.html", "guides/index.html", "start-here/index.html"]:
    ...
    def test_idempotence_and_unpublished_state(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            report = json.loads((site / "api" / "accessible-mental-health-appointments-v194.json").read_text(encoding="utf-8"))
