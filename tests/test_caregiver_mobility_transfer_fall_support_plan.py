...
CONTENT = Path("learning-paths/caregiver-foundations/mobility-transfer-and-fall-observation-support-plan.json")
...
def test_methodological_depth():
    ...
    assert len(d["event_log_fields"]) >= 14
...
def test_safety_boundaries_are_explicit():
    ...
    for phrase in [
        "لا تشخيص منزلي",
        "لا تغيير جرعات الأدوية",
        "لا تعليم أو تنفيذ مناورة رفع",
        "خدمات الطوارئ المحلية",
        "لا استخدام حزام أو تقييد",
        "لا تعني الإحالة إلى WHO أو NICE",
    ]:
        assert phrase in text


def test_sources_are_official_and_scope_limited():
    d = load()
    assert len(d["sources"]) >= 6
    urls = [s["url"] for s in d["sources"]]
    assert sum("who.int" in u for u in urls) >= 4
    assert any("nice.org.uk" in u for u in urls)
    assert any("un.org" in u for u in urls)
    assert all(u.startswith("https://") for u in urls)
    nice = next(s for s in d["sources"] if "nice.org.uk" in s["url"])
    assert "50–64" in nice["scope"] and "لم يُعمم" in nice["scope"]
...
def test_internal_links_connect_related_caregiver_packages():
    ...
    assert any("sensory-environment-and-routine-adjustment-plan" in x["url"] for x in links)
...
def test_original_arabic_content_not_placeholder():
    ...
    assert text.count("السقوط") >= 20
    assert text.count("الاستقلال") >= 6
