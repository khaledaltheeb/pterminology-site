...
class ChooseProfessionalProductionV178Tests(unittest.TestCase):
    ...
    def test_core_publisher_preserves_extensions_and_separates_source_from_published(self):
        ...
        self.assertIn('"source_guides": len(all_guides)', text)
        self.assertIn('"core_guides": len(all_guides)', text)
        self.assertIn('"published_core_guides": len(guides)', text)
        ...
        self.assertIn('BLOCKED_REVIEW_STATUSES = {"needs-specialist-review"}', text)
        self.assertIn('"needs_specialist_review_published": False', text)
