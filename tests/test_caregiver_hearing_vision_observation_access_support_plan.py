...
CONTENT = Path("learning-paths/caregiver-foundations/hearing-vision-observation-and-access-support-plan.json")
...
def test_methodological_depth():
    ...
    assert len(d["sensory_log_fields"]) >= 14
...
def test_safety_boundaries_are_explicit():
    ...
    for phrase in [
        "لا تشخيص منزلي لفقدان السمع",
        "لا تستخدم أصواتًا شديدة",
        "لا تُدخل أعوادًا أو أدوات",
        "لا تبدأ قطرات أذن أو عين",
        "لا تغيّر وصفة النظارة",
        "لا تمنع وسيلة تواصل",
...
def test_sources_are_current_official_and_scope_limited():
    ...
    assert sum("who.int" in u for u in urls) >= 3
    ...
    assert sum(s["date"].startswith("2026") for s in d["sources"]) >= 2
...
def test_internal_links_connect_related_caregiver_packages():
    ...
    assert any("professional-visit-and-shared-decision" in x["url"] for x in links)
...
def test_original_arabic_content_not_placeholder():
    ...
    assert text.count("السمع") >= 30
    assert text.count("البصر") >= 20
