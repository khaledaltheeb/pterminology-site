...
SCRIPT = ROOT / "scripts" / "publish_arabic_plain_language_v193.py"

spec = importlib.util.spec_from_file_location("plain_language_v193", SCRIPT)
module = importlib.util.module_from_spec(spec)
assert spec.loader is not None
spec.loader.exec_module(module)


class ArabicPlainLanguageGuideTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.data = json.loads(CONTENT.read_text(encoding="utf-8"))

    def test_content_depth_and_editorial_limits(self):
        ...
        for marker in ["المصطلح", "H1", "الميتا", "نص كل رابط", "الحمل المعرفي", "built-not-published"]:
            self.assertIn(marker, text)
        for forbidden in ["يضمن الشفاء", "شخّص نفسك", "مناسب للجميع"]:
            self.assertNotIn(forbidden, self.data["summary"] + self.data["description"])
    ...
    def test_render_metadata_schema_and_contextual_links(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for marker in ['lang="ar" dir="rtl"', 'rel="canonical"', 'name="robots" content="index,follow"', 'property="og:title"', 'name="twitter:card"', '"@type": "Article"', '"@type": "BreadcrumbList"', '"citation"', ':focus-visible', '@media print']:
                self.assertIn(marker, page)
            href = "/guides/arabic-plain-language-for-mental-health-content/"
            for relative in ["encyclopedia/index.html", "blog/index.html", "special-needs/index.html"]:
                self.assertIn(href, (site / relative).read_text(encoding="utf-8"))
            canonical = module.BASE_URL + href
    ...
    def test_idempotence_and_unpublished_state(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            report = json.loads((site / "api" / "arabic-plain-language-v193.json").read_text(encoding="utf-8"))
