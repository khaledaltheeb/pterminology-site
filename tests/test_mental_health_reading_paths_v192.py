...
SCRIPT = ROOT / "scripts" / "publish_mental_health_reading_paths_v192.py"

spec = importlib.util.spec_from_file_location("reading_paths_v192", SCRIPT)
module = importlib.util.module_from_spec(spec)
assert spec.loader is not None
spec.loader.exec_module(module)


class MentalHealthReadingPathsTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_content_depth_and_limits(self):
        ...
        for marker in ["نية القارئ", "نص الرابط", "الصفحات اليتيمة", "BreadcrumbList", "ItemList", "canonical", "built-not-published"]:
    ...
    def test_render_links_and_schema(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for marker in ['rel="canonical"', 'name="robots" content="index,follow"', 'property="og:title"', 'name="twitter:card"', '"@type": "Article"', '"@type": "BreadcrumbList"', '"@type": "ItemList"', '"citation"', ':focus-visible', '@media print']:
                self.assertIn(marker, page)
            href = "/guides/mental-health-reading-paths/"
            for relative in ["encyclopedia/index.html", "blog/index.html", "special-needs/index.html"]:
    ...
    def test_idempotence_and_unpublished_state(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            report = json.loads((site / "api" / "mental-health-reading-paths-v192.json").read_text(encoding="utf-8"))
