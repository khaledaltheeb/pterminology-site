...
def add_to_sitemap(urls: list[str]) -> None:
    ...
    if tag == "sitemapindex":
        ...
        for url in urls:
            if url in existing:
                continue
            node = ET.SubElement(current, f"{{{ns}}}url")
            ET.SubElement(node, f"{{{ns}}}loc").text = url
            ET.SubElement(node, f"{{{ns}}}lastmod").text = "2026-07-22"
        tree.write(index, encoding="utf-8", xml_declaration=True)
    else:
        raise SystemExit(f"Unsupported sitemap root: {tag}")


def source_contract_ready(source: object) -> bool:
    return isinstance(source, dict) and SOURCE_FIELDS <= set(source) and isinstance(source.get("claims_supported"), list) and bool(source["claims_supported"])
...
def main() -> None:
    ...
    for article in articles:
        ...
        sections = "".join(
            f'<section><h2>{e(section["heading"])}</h2>'
        ...
        sources = "".join(
            f'<li><a href="{e(source["url"])}" rel="external noopener">{e(source["publisher"])}: {e(source["title"])}</a> — {e(source["year"])}، تحقق {e(source["verified_at"])}</li>'
            for source in article["sources"]
        )
        source_records.extend(article["sources"])
    ...
    write_page(
        ...
        BASE + "blog/",
        index_body,
        {"@context": "https://schema.org", "@type": "Blog", "name": collection["title"], "description": collection["description"], "inLanguage": "ar", "url": BASE + "blog/"},
    )
    add_to_sitemap(urls)
    ...
    report = {
        ...
        "urls": urls,
        "reviewed": all(article.get("status") == "reviewed" for article in articles),
        "source_records": len(source_records),
        "unique_source_ids": len(set(source_ids)),
        "source_contract_ready": bool(source_records) and all(source_contract_ready(source) for source in source_records),
        "source_ids": source_ids,
    }
    if report["unique_source_ids"] != report["source_records"]:
        raise SystemExit("Duplicate source ids in blog data")
    if not report["source_contract_ready"]:
        raise SystemExit("Blog source records do not satisfy the source evidence contract")
    api = SITE / "api"
    api.mkdir(parents=True, exist_ok=True)
    (api / "blog-v161.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
