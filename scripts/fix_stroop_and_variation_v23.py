...
def main()->None:
    ...
    new_return="return{prompt,options,answer,kind,explanation,audioCount,delay,study:typeof study==='undefined'?'':study,studyMs:typeof studyMs==='undefined'?0:studyMs,stimulusWord:typeof stimulusWord==='undefined'?'':stimulusWord,stimulusInk:typeof stimulusInk==='undefined'?'':stimulusInk,stimulusRule:typeof stimulusRule==='undefined'?'':stimulusRule};"
    text=replace_once(text,old_return,new_return,'trial metadata return')
    JS.write_text(text,encoding='utf-8')
    report={'version':23,'stroop_basic_varies':True,'stroop_advanced_varies':True,'ink_rendered_inline':True,'first_five_guard_added':True}
    api=SITE/'api';api.mkdir(exist_ok=True);(api/'stroop-variation-v23.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
