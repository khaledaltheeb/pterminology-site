...
INDEX_JS = r'''/* v14 encyclopedia paginated search */
(()=>{'use strict';
...
function render(){const grid=q('#ency-results'),count=q('#ency-count'),status=q('#ency-status'),pages=Math.max(1,Math.ceil(filtered.length/PAGE_SIZE));page=Math.min(page,pages);const start=(page-1)*PAGE_SIZE,end=Math.min(filtered.length,start+PAGE_SIZE);grid.innerHTML=filtered.slice(start,end).map(card).join('')||'<p class="ency-v14__empty">لا توجد نتائج مطابقة. جرّب كلمة أقصر أو أزل أحد المرشحات.</p>';count.textContent=filtered.length.toLocaleString('ar');status.textContent=`عرض ${filtered.length?start+1:0}–${end} من ${filtered.length.toLocaleString('ar')} · الصفحة ${page} من ${pages}`;q('#ency-prev').disabled=page<=1;q('#ency-next').disabled=page>=pages;window.requestAnimationFrame(()=>grid.setAttribute('aria-busy','false'))}
function apply(){if(!loaded)return;const term=normalize(q('#ency-q').value.trim()),domain=q('#ency-domain').value,category=q('#ency-category').value;filtered=all.filter(x=>(!term||normalize(`${x.ar} ${x.en} ${x.category} ${x.facet} ${x.domain}`).includes(term))&&(!domain||x.domain===domain)&&(!category||x.category===category));page=1;q('#ency-results').setAttribute('aria-busy','true');render()}
async function load(){try{const response=await fetch(`${BASE}api/encyclopedia-v13.json`,{cache:'force-cache'});if(!response.ok)throw new Error(`HTTP ${response.status}`);const data=await response.json();all=Array.isArray(data.items)?data.items:[];filtered=all;loaded=true;apply()}catch(error){q('#ency-results').innerHTML='<p class="ency-v14__empty">تعذر تحميل فهرس البحث الآن. ما زالت الصفحات متاحة من المراكز الموضوعية وخريطة الموقع.</p>';q('#ency-status').textContent='تعذر تحميل البحث';console.error(error)}}
...
#ency-results[aria-busy="true"]{opacity:.65}
@media(max-width:640px){.ency-v14__status{display:block}.ency-v14__pager{position:sticky;bottom:8px;z-index:8;padding:8px;border-radius:999px;background:rgba(255,255,255,.92);backdrop-filter:blur(10px)}}
'''
...
def build_lightweight_index() -> dict[str, int]:
    ...
    body = f'''<!doctype html><html lang="ar" dir="rtl"><head>{head}</head><body><main class="ency-v13"><header class="ency-v13__hero"><h1>الموسوعة النفسية العربية</h1><p>مئة موضوع أساسي وعشرون زاوية تحريرية لكل موضوع، مع فهرس سريع لا يحمّل آلاف البطاقات دفعة واحدة.</p><p><strong id="ency-count">2000</strong> صفحة مستقلة موجودة في خريطة الموقع والمراكز الموضوعية.</p><div class="ency-v13__search"><label>البحث<input id="ency-q" type="search" inputmode="search" autocomplete="off" placeholder="ابحث بالعربية أو الإنجليزية"></label><label>الموضوع<select id="ency-domain"><option value="">كل الموضوعات</option>{domain_options}</select></label><label>التصنيف<select id="ency-category"><option value="">كل التصنيفات</option>{category_options}</select></label></div></header><section class="ency-v14__status"><span id="ency-status" role="status" aria-live="polite">جارٍ تحميل الفهرس الخفيف…</span><a href="{BASE}hubs/">التصفح عبر 200 مركز موضوعي</a></section><section id="ency-results" class="ency-v13__grid" aria-busy="true"><p class="ency-v14__empty">جارٍ تحميل النتائج…</p></section><nav class="ency-v14__pager" aria-label="صفحات نتائج الموسوعة"><button id="ency-prev" type="button" disabled>السابق</button><button id="ency-next" type="button" disabled>التالي</button></nav></main><noscript><main class="ency-v13"><p class="ency-v14__empty">البحث التفاعلي يحتاج JavaScript. استخدم <a href="{BASE}hubs/">المراكز الموضوعية</a> لتصفح جميع الموضوعات.</p></main></noscript><script src="{BASE}assets/js/encyclopedia-v14.js" defer></script><script src="{BASE}assets/js/app-v10.js" defer></script></body></html>'''
    ...
    return {"items": len(items), "domains": len(domains), "categories": len(categories)}
...
def main() -> None:
    ...
    checks = {
        ...
        "no_2000_static_cards": page.count("ency-v13__card") == 0,
    ...
    report = {"version": 14, "removed_lab_script_tags": removed, "kept_lab_script_tags": kept, **index, **checks}
