...
SITEMAP_NAME = "sitemap-psychology-term-disambiguation.xml"
...
def render(data: dict) -> str:
    ...
    defined_term = {
        "@context": "https://schema.org",
        "@type": "DefinedTerm",
        "name": "المصطلح النفسي العربي",
        "description": data["summary"],
        "inDefinedTermSet": f"{BASE_URL}/encyclopedia/",
    }
    sections = "".join(
        f'<section><h2>{esc(section["heading"])}</h2>' + "".join(f"<p>{esc(paragraph)}</p>" for paragraph in section["paragraphs"]) + "</section>"
    ...
    schemas = json.dumps([article, breadcrumb, defined_term], ensure_ascii=False)
...
<section><h2>المصادر وما تدعمه</h2><p>الحالة التحريرية: مراجعة داخلية. آخر مراجعة: {esc(data["reviewed_at"])}.</p><ul>{sources}</ul></section>
</main></body></html>'''
...
def add_contextual_link(path: Path, href: str, label: str) -> None:
    ...
    if href in text:
        return
    link = f'<p class="term-disambiguation-guide-link"><a href="{href}">{esc(label)}</a></p>'
...
def publish(site: Path) -> Path:
    ...
    add_contextual_link(site / "encyclopedia" / "index.html", href, "دليل توحيد المصطلحات والمرادفات")
    add_contextual_link(site / "blog" / "index.html", href, "دليل منع تضارب المصطلحات والعناوين")
    add_contextual_link(site / "special-needs" / "index.html", href, "ضبط المصطلحات واللغة المرجعية")
    ...
    report = site / "api" / "psychology-term-disambiguation-v195.json"
    report.parent.mkdir(parents=True, exist_ok=True)
    report.write_text(json.dumps({"status": "built-not-published", "version": 195, "path": href, "sitemap": f"/{SITEMAP_NAME}", "review": data["status"], "risk_level": data["risk_level"]}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return output
