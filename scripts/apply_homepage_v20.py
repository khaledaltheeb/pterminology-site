...
def main() -> None:
    ...
    subprocess.run([sys.executable, str(ROOT / "scripts" / "publish_blog_v160.py"), str(SITE)], check=True)
    subprocess.run([sys.executable, str(ROOT / "scripts" / "finalize_blog_links_v160.py"), str(SITE)], check=True)
