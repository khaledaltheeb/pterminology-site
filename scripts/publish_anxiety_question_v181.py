...
def render_page(data: dict) -> str:
    ...
    schemas = [
        {
            ...
            "isPartOf": {"@type": "WebSite", "name": "مصطلحات علم النفس", "url": "https://khaledaltheeb.github.io/pterminology-site/"},
        },
    ...
    faqs = "".join(
        f'<details><summary>{esc(item["question"])}</summary><p>{esc(item["answer"])}</p></details>' for item in data["faq"]
    )
    links = "".join(f'<li><a href="{esc(item["href"])}">{esc(item["label"])}</a></li>' for item in data["internal_links"])
    sources = "".join(
        f'<li><a href="{esc(item["url"])}" rel="noopener noreferrer">{esc(item["publisher"])} — {esc(item["title"])}</a> <span>({esc(item["year"])})</span></li>'
        for item in data["sources"]
    )
    schema_json = "".join(f'<script type="application/ld+json">{json.dumps(schema, ensure_ascii=False)}</script>' for schema in schemas)
...
def publish(site_dir: Path) -> dict:
    ...
    report = {"publisher": data["publisher_id"], "item_id": data["item_id"], "route": data["route"], "state": "built-not-published", "page": output.relative_to(site_dir).as_posix(), "sitemap": sitemap.name, "source_count": len(data["sources"]), "live_verified": False}
...
def main() -> int:
    ...
    print(json.dumps(publish(args.site_dir), ensure_ascii=False))
    return 0
